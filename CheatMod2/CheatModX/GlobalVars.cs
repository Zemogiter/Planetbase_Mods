...
public class GlobalVars : Singleton<GlobalVars>
{
	...
	public void ModSaveData(XmlNode rootNode)
	{
		...
		Serialization.serializeString(parent, "Cantidad", text4);
		Serialization.serializeFloat(parent, "Tiempo", fTime);
		Serialization.serializeBool(parent, "BTiempo", bTime);
	}
	...
	public void ModLoadData(XmlNode node)
	{
		try
		{
			...
			fTime = Serialization.deserializeFloat(node["Tiempo"]);
			bTime = Serialization.deserializeBool(node["BTiempo"]);
		}
		catch
		{
		}
